	...
	<div id="footer" class="pagewidth clearfix">
		...
		<div class="footer-text">
			...
			if(themify_get('setting-footer_text_left') != "") {
				...
				echo '&copy; <a href="'.get_option('home').'">'.get_bloginfo('name').'</a> '.date('Y') . '<br />';
			} ?>
			<?php
			if(themify_get('setting-footer_text_right') != "") {
				echo themify_get('setting-footer_text_right');
			} else {
				echo 'Powered by <a href="http://wordpress.org">WordPress</a> &bull; <a href="http://themify.me">Themify WordPress Themes</a>';
			} ?>
		</div>
		<!--/footer-text -->
