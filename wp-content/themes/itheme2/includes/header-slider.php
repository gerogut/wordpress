			...
			if(themify_check('setting-header_slider_display') && themify_get('setting-header_slider_display') == "images"){ 
					...
					if(themify_check('setting-header_slider_images_'.$option.'_image')){
						echo '<li>';
							if(themify_check('setting-header_slider_images_'.$option.'_link')){ 
								...
								themify_image("src=".$image."&setting=header_slider&w=220&h=160&before=<div class='slide-feature-image'><a href='".urlencode($link)."' title='".urlencode($link)."'>&after=</a></div>&alt=".urlencode($link)."&class=feature-img");
								echo $title;
								
							} else {
								
								$image = themify_get('setting-header_slider_images_'.$option.'_image');
								themify_image("src=".$image."&setting=header_slider&w=220&h=160&alt=".$image."&class=feature-img&before=<div class='slide-feature-image'>&after=</div>");
							}
						echo '</li>';
					}
				}
				...
				if( have_posts() ) {
					...
					<?php endif; ?>
						...
						<?php themify_image("field_name=feature_image, post_image, image, wp_thumb&ignore=true&setting=header_slider&w=220&h=160&class=feature-img&before=<div class='slide-feature-image'><a href='".urlencode($link)."'>&after=</a></div>"); ?>
						<!-- /.slide-feature-image -->
